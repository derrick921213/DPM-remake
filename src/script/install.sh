...
if exists_in_list "$os_like" " " debian; then
    ...
    sudo dnf update -y &&  sudo dnf install epel-release -y && sudo dnf install python3 patchelf git python3-devel yum-utils make gcc openssl-devel bzip2-devel libffi-devel zlib-devel -y
else
    exit 1
fi
